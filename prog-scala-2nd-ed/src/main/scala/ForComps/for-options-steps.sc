...
// A sequence of functions for the process steps. Each takes the result of the previous step (or a seed value to start) and returns a new option.
...
// Use +flatMap+ to extract the current sum value and pass it to the current step, which returns a new +Option+ that becomes the next +sumOpt+. If +sumOpt+ is actually a +None+, the anonymous function isn't called and +None+ is returned.
