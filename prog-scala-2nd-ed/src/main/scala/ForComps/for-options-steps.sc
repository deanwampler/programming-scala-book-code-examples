...
// A function to combine to Options using a function their values.
def optionMap2[T1,T2,T3](                              // <1>
    opt1: Option[T1], opt2: Option[T2])(f: (T1,T2) => T3): Option[T3] = 
  (opt1, opt2) match {
    case (None, _)            => None                  // <2>
    case (_, None)            => None
    case (Some(t1), Some(t2)) => Some(f(t1, t2))       // <3>
  }

def sumCounts1(counts: List[Option[Int]]): Option[Int] = 
  (counts foldLeft Option(0)) {                        // <4>
...
def sumCounts2(counts: List[Option[Int]]): Option[Int] = {   // <6>
