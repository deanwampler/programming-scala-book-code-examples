  ...
  describe ("Payroll calculation") {
    it ("calculate the gross, net, and deductions for the pay period") {
      for (m <- 3 to 10) {
        ...
        actual.gross must beCloseTo(expectedGross, Money(0.001))
        actual.net must beCloseTo(expectedNet, Money(0.001))
        actual.deductions must beCloseTo(expectedDeductions, Money(0.001))
      }
    }
  }
}
