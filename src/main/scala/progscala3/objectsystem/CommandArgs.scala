...
object CommandArgs:
  ...
  def parseParams(params: Array[String]): Args =
    def pa(params2: List[String], result: Args): Args = params2 match  // <3>
      case Nil => result                                               // <4>
      ...
      case _ => quit(s"Unrecognized argument ${params2.head}", 1)      // <9>

    val argz = pa(params.toList, Args("", ""))                         // <10>
    if argz.inputPath == "" || argz.outputPath == "" then              // <11>
      quit("Must specify input and output paths.", 1)
    argz

  def main(params: Array[String]): Unit =
    val argz = parseParams(params)
    println(argz)
